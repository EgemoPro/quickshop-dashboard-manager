...
export default function AuthForm({
  ...
  const handlePasswordlessSubmit = useCallback(async (data: z.infer<typeof passwordlessSchema>) => {
    setIsLoading(true);
  ...
  const handleLoginSubmit = useCallback(async (data: z.infer<typeof loginSchema>) => {
    setIsLoading(true);
    try {
      if (onSubmit) {
  ...
  const handleRegisterSubmit = useCallback(async (data: z.infer<typeof registerSchema>) => {
    setIsLoading(true);
    try {
      if (onSubmit) {
  ...
  const logoComponent = useMemo(() => {
    return logo || <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-primary" />
          <span className="text-xl font-bold">{title}</span>
        </div>;
  }, [logo, title]);
  return <div className={cn("grid min-h-screen w-full lg:grid-cols-2", className)}>
      {/* Image Section */}
      <div className="relative hidden lg:block">
        <img src={imageSrc || "/placeholder.svg"} alt={imageAlt} className="absolute h-full inset-0 object-cover" />
        <div className="absolute inset-0 bg-black/20" />
        <div className="absolute left-8 top-8 z-20 max-md:bg-violet-800">
          {logo || <div className="flex items-center gap-2 text-white">
      ...
      <div className="flex items-center justify-center overflow-hidden  max-md:p-8">
        ...
        duration: 0.5
          ...
          <Card className="max-md:mt-8">
            ...
            <CardContent>
              {isSuccess ? <SuccessMessage /> : <Tabs defaultValue="passwordless" value={activeTab} onValueChange={value => setActiveTab(value as any)} className="w-full">
                  ...
                  <TabsContent value="passwordless">
                    <PasswordlessLoginForm form={passwordlessForm} onSubmit={handlePasswordlessSubmit} onSocialLogin={handleSocialLogin} isLoading={isLoading} />
                  </TabsContent>

                  {/* Traditional Login Tab */}
                  <TabsContent value="login">
                    <LoginForm form={loginForm} onSubmit={handleLoginSubmit} onSocialLogin={handleSocialLogin} isLoading={isLoading} />
                  </TabsContent>

                  {/* Registration Tab */}
                  <TabsContent value="register">
                    <RegisterForm form={registerForm} onSubmit={handleRegisterSubmit} onSocialLogin={handleSocialLogin} isLoading={isLoading} />
                  </TabsContent>
                </Tabs>}
            </CardContent>
...
function PasswordlessLoginForm({
  ...
  return <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        field
      }) => <EmailInput field={field} disabled={isLoading} />} />
...
function LoginForm({
  ...
  return <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        field
      }) => <PasswordInput label="Password" field={field} disabled={isLoading} />} />
...
function RegisterForm({
  ...
  return <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        field
      }) => <PasswordInput label="Confirm Password" field={field} disabled={isLoading} />} />
